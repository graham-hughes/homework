...
class QLearner(object):
    ...
    lander=False):
        ...
        # Evaluate the actual q value for the given self.q_t and act_t_ph
        q_value = tf.reduce_sum(tf.multiply(self.q_t, tf.one_hot(self.act_t_ph, self.num_actions)), axis = 1)
    ...
    def step_env(self):
        ...
        #####
        # Store the last observation in the replay buffer
        idx = self.replay_buffer.store_frame(self.last_obs)

        # Pick random action to initialize or perform epsilon greedy exploration
        if not self.model_initialized or self.exploration.value(self.t) > np.random.uniform():
            action = self.env.action_space.sample()
        else:
            # Get most recent observations
            recent_observations = self.replay_buffer.encode_recent_observation()
            # Run the current q_t on the most recent observations
            action_values = self.session.run(self.q_t, feed_dict={self.obs_t_ph: np.expand_dims(recent_observations, axis=0)})
        ...
        # Update value in replay buffer
        self.replay_buffer.store_effect(idx, action, rew, done)

        # Replace latest observation
        if done:
            self.last_obs = self.env.reset()
        else:
            self.last_obs = obs
